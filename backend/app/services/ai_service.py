    ...
    async def answer_with_tools(
        ...
        *,
        ...
        chat_system = (
            ...
            "the previously proposed event(s) (create_calendar_event). Use the email tools whenever "
            "the user asks something specific — do not guess and do not fall back to a generic "
            "inbox summary if the answer requires looking at actual email content. search_emails "
            "returns source/mailbox metadata and may include live Gmail fallback results. If it "
            "returns no results, say you couldn't find a matching message in the places searched; "
            "do not claim the email definitely does not exist.\n\n"
            "Prior assistant turns in this session reflect actions the system has already taken. "
            "Use them as context for follow-up actions. Do NOT re-run search_emails or "
            "get_email to re-verify details you already gave the user. Re-search only if the "
            "user is asking about something genuinely new or you do not yet have the detail.\n\n"
            "Calendar event flow (STRICT — the server enforces this):\n"
            "  Turn 1: gather the details (re-use any details from earlier in the conversation "
            "before reaching for search_emails / get_email). Then call propose_calendar_event "
            "ONCE PER EVENT. For 'add both' style requests, call propose_calendar_event multiple "
            "times in the same turn — once for each event — before replying. This only records "
            "an internal pending calendar offer. Do not mention staging, queuing, pending state, "
            "or any technical step. Describe the event details and ask whether the user wants "
            "you to add it to their calendar.\n"
            "  Turn 2: ONLY after the user clearly confirms ('yes', 'go ahead', 'book it', 'add "
            "to calendar', 'add both'), call create_calendar_event (no arguments — the server "
            "creates all pending proposals). To create just one of several pending proposals, "
            "pass the proposal_id returned by propose_calendar_event. The server will reject "
            "the call if there is no pending proposal or if the user has not clearly confirmed.\n"
            "Never call create_calendar_event on the same turn as propose_calendar_event.\n"
            "Once create_calendar_event returns ok, do not propose the same event again — it is "
            "already on the calendar.\n\n"
            "For calendar events, infer title, time, duration, and attendees from the email or "
            "from what you already told the user. If the time isn't pinned down anywhere, ask — "
            "never invent one. Default duration is 60 minutes if unstated.\n\n"
