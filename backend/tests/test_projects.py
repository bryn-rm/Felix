...
async def project_db(monkeypatch, request):
    ...
    try:
        ...
        project_migration = ROOT / ("infra/migrations/025_project_questions.sql" if mode.startswith("phase4") else
                                    "infra/migrations/024_project_suggestions.sql" if mode.startswith("phase3") else
        ...
        prior_project = None
        if mode in {"phase2-upgrade", "phase3-upgrade", "phase4-upgrade"}:
