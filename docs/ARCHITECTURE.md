...
Unauthenticated visitors to `/` see the public Dossier homepage; validated signed-in users still redirect to `/home`. `/login` retains the existing Google sign-in and return-path flow. Both public and login pages use `NEXT_PUBLIC_REQUEST_ACCESS_URL` for the external access form and hide request links when it is unset. The public homepage uses a scoped CSS module and `next/font/local` fonts under `components/homepage`, with vendored licenses and provenance in `fonts/README.md`. Its viewport and document canvas use cream while the public page is present; other routes retain the app theme. `NEXT_PUBLIC_SITE_URL` supplies the canonical company origin for canonical/Open Graph URLs and the existing Felix icon used by summary social cards, falling back to `VERCEL_PROJECT_PRODUCTION_URL`. Without either origin, the page emits text social metadata and omits absolute URLs/images.
